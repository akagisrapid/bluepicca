...
public func uploadBlob(imageData: Data, mimeType: String) async throws -> UploadBlobResponse {
  ...
  dlog("アップロード予定の画像サイズ: \(fileSizeString)")

  // ファイルサイズ制限チェック
  if ImageCompressionHelper.isFileSizeExceeded(imageData) {
    dlog("警告: ファイルサイズが制限を超えています: \(fileSizeString)")
    throw NSError(
      domain: "UploadError", code: 413,
      userInfo: [
        NSLocalizedDescriptionKey:
          "ファイルサイズが大きすぎます (\(fileSizeString)). 最大サイズは\(ImageCompressionHelper.formatFileSize(ImageCompressionHelper.maxFileSizeBytes))です。"
      ])
  }
  ...
  do {
      ...
      do {
        if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
          ...
          if let blob = json["blob"] as? [String: Any] {
            ...
            {
              dlog("手動解析 - cid: \(cid), mimeType: \(mimeType)")
    ...
    case .success(let value):
      dlog("画像アップロード成功: \(value)")

      // CIDの形式を確認
      dlog("CIDの形式: \(value.blob.cid)")
      dlog("CIDの長さ: \(value.blob.cid.count)")
      dlog("CIDのプレフィックス: \(value.blob.cid.prefix(4))")

      // CIDがbase32またはbase58btcエンコードされているか確認
      if value.blob.cid.hasPrefix("bafy") || value.blob.cid.hasPrefix("bafk") {
        dlog("CIDは正しい形式です")
      } else {
        dlog("警告: CIDがbase32またはbase58btcエンコードされていない可能性があります")
        dlog("CID全体: \(value.blob.cid)")
      }
    ...
    case .failure(let error):
      ...
      dlog("ステータスコード: \(response.response?.statusCode ?? 0)")
      dlog("エラー: \(error)")
